...
def get_package_id(request):
    try:
        if request.method == "GET":
            ...
            package_id = request.POST.get('packageid', None)
        return package_id
...
class Authtication(BasicAuthentication):
    ...
    def authenticate(self, request):
        ...
        try:
            ...
            get_app_cache_key = f"api_{user_uid}_{package_id}_get_app_v2"
            app = cache.get(get_app_cache_key)
            if app is None:
                try:
                    app = ApplicationsModelV2.objects.get(user_uid=user_uid, packageid=package_id)                
                    cache.set(get_app_cache_key, app,settings.API_CACHE_EXPIRED)
                except ApplicationsModelV2.DoesNotExist:
...
class AuthticationWithoutPackageId(BasicAuthentication):
    ...
    def authenticate(self, request):
        ...
        try:
            ...
            if app is None:
                try:
                    app = ApplicationsModelV2.objects.get(
